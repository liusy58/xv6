...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
  ...
  for(a = oldsz; a < newsz; a += PGSIZE){
    ...
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_W|PTE_X|PTE_R|PTE_U) != 0){
      ...
      kpte = walk(myproc()->kpagetable,a,1);
...
void copy_user2kernel(pagetable_t pagetable,pagetable_t kpagetable,uint64 sz){
  pte_t *pte,*kpte;
  uint64 pa, i;
  uint flags;
  for(i = 0; i < sz; i += PGSIZE){
    if((pte = walk(pagetable, i, 0)) == 0)
      panic("copy_user2kernel: pte should exist");
    if((*pte & PTE_V) == 0)
      panic("copy_user2kernel: page not present");
    pa = PTE2PA(*pte);    
    flags = PTE_FLAGS(*pte)&(~PTE_U);
